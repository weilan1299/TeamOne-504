...
# Viewer or displayer, interacts with the filewatch, and the filewatch
# will ask databasemanager to insert new data about the file we are watching.
# The databasemanager will notify the Tkiner to update status/data
...
class Tkinter_GUI(Observer):
    ...
    def db_write(self):
        ...
        cursor = conn.cursor()
        cursor.execute("INSERT INTO events (filename, path, event_type, timestamp) VALUES (?, ?, ?, ?)",
                       (self.filename.get(), self.path.get(), self.event.get(), self.time.get()))
    ...
    def show_usage(self):
        ...
        A) Top Left Corner: The window has four small buttons, named as "File", "Edit", 
            ...
            3. Database: Clicking this button will allow user/you to see small dropdown button,
                consisting "Write","Clear database", "Delete database", "Change database", and "Query".
        ...
        B) Where to watch: First a user/you should determine which directory, or
            ...
            4. Change Target Directory: To change the directory the user/you want to watch, 
        ...
        C) Database Management: The user have the following options for data management.
            ...
            2. Click the "Write" button either bellow the Database path or from the "Database" 
                on the top-left corner. This will allow the user to write the data to a specified
                file or directory/ies.
                
            3. Click the "Clear DB" or "Clear database" buttons to clear the output/result of 
                the watched files before the result is stored to an specified file or directory/ies.
                
            4. Click the "Delete database" under the "Database" button to delete stored 
                output/result data.
                
            5. Click the "Change database" under the "Database" button to modify stored 
                output/result data.
                
            6. Click the "Query" button under the "Database" button or the "Query" button to 
            ...
            7. File Watcher System Events will appear the last box of the window and will 
                have a File Name, Path, Event Type, and Time Stamp in real-time occurrence.
        """
        messagebox.showinfo("Usage Help: ", usage_message)
        print(usage_message)
