...
def start_watch(path):
    filewatch = FileWatch(textbox, path)
    filewatch.start()

start_button = Button(frame2, text="Start",bg="#28393a", fg="white", cursor="hand2", activebackground="#badee2", activeforeground="black", command=lambda: start_watch(entry_var.get()))
...
# textbox = CTkTextbox(frame4, wrap="word", width=480, height=250)
# textbox.grid(row=2, column=0, columnspan=2,sticky="e", padx=100, pady=5)
